...
let resizeAndUpload = (imageUrl, imageName, resizeMediumDirectory, resizeSmallDirectory) => {
    ...
    return new Promise(function (resolve, reject) {
        ...
        jimp.read(imageUrl).then(imageToResize => {
            imageToResize.resize(100, jimp.AUTO).quality(100).write(resizedFilePath, function () {
                uploadResizedImage(resizedFilePath, resizeSmallDirectory, imageName).then(doneUploading => {
        ...
        jimp.read(imageUrl).then(imageToResize => {
            imageToResize.resize(300, jimp.AUTO).quality(100).write(resizedMediumFilePath, function () {
                uploadResizedImage(resizedMediumFilePath, resizeMediumDirectory, imageName).then(doneUploading => {
